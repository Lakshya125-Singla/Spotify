...
const playMusic = (track, pause = false) => {
    ...
    if (!pause) {
        currentSong.play()
            ...
            .catch(err => {
                console.warn("Playback error in playMusic:", err);
            });
    }
...
// ... existing code
    ...
    for (let index = 0; index < array.length; index++) {
        ...
        if (e.href.includes("/songs") && !e.href.includes(".htaccess")) {
            ...
            console.log("Parsed folder:", folder, "Full href:", e.href);

            // Get the metadata of the folder
            let data;
            try {
                let res = await fetch(`/songs/${folder}/info.json`)
                if (!res.ok) continue; // Skip if not found
                data = await res.json();
    ...
    Array.from(document.getElementsByClassName("card")).forEach(e => {
        e.addEventListener("click", async item => {
            const folder = e.dataset.folder;  // ✅ FIX
...
async function main() {
    ...
    if (isNaN(currentSong.duration)) return; // ✅ Prevent setting invalid time
